...
- Secure PDF export contracts under `apps/backend/src/pdf-exports`, including authenticated export/download routes, Step 8 entitlement checks, ready-book snapshot validation, deterministic content/layout versions, in-process idempotency plus database-backed duplicate-render avoidance, escaped semantic print HTML, owner/book-scoped private image asset loading with deterministic fallbacks, a hardened browser-renderer port that disables JavaScript and aborts external requests, private owner/book-scoped PDF storage keys, short-lived signed download responses, sanitized failure mapping, and fake/local-port tests
...
- Public-template publication, deployment, refunds/proration/customer portal/multi-plan billing, production Stripe account/Price setup, production PDF renderer/storage wiring, and QA product behavior scheduled for later steps
...
- Scope boundary: Step 9 does not implement AI regeneration, billing changes, public-template publication, reader redesign, social/public sharing, production deployment, real S3 adapter wiring, or a bundled Chromium runtime. The local private-object adapter requires explicit local env configuration; the browser renderer remains fail-closed until production Chromium wiring is configured.
- Verification status: sanitized Docker gates passed without mounting repository-root `.env`: root `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run test:coverage`; backend coverage reported 101 tests across 26 files with 79.09% lines and 65.72% branches, and frontend coverage reported 16 tests across 5 files with 83.01% lines and 78.94% branches. Focused Step 9 backend verification also passed 5 PDF export test files and 23 tests. Production `npm audit --omit=dev --json` currently reports three high, zero critical findings through `prisma`/`@prisma/config`/`deepmerge-ts`; Step 9 did not change package manifests or lockfiles, npm reports latest Prisma as 7.9.1, and npm's available fix is an unsafe major downgrade to Prisma 6.12.0, so this must be tracked as a current upstream dependency-security issue rather than silently remediated in the PDF export story.
