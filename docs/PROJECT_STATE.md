...
- Secure PDF export contracts under `apps/backend/src/pdf-exports`, including authenticated export/download routes, Step 8 entitlement checks, ready-book snapshot validation, deterministic content/layout versions, in-process idempotency plus database-backed duplicate-render avoidance, stale pending retry recovery, escaped semantic print HTML, owner/book-scoped private image asset loading with deterministic fallbacks and dimension bounds, a real Puppeteer-backed hardened browser renderer that disables JavaScript and aborts external requests, configured render concurrency limiting, private owner/book-scoped PDF storage keys, short-lived signed download responses, sanitized failure mapping, fake/local-port tests, and opt-in real Chromium PDF inspection tests
...
- Added configured render concurrency limiting, stale pending export reclaim after `PDF_EXPORT_PENDING_STALE_MS`, and PNG/JPEG/WebP image dimension/pixel validation before Chromium decode/layout.
