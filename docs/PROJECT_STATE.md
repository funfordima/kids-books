...
- Pinned backend PDF renderer runtime Dockerfile under `infra/docker/backend-pdf-runtime.Dockerfile`, with Chromium executable path configuration and `.dockerignore` protection for local `.env`
...
- Public-template publication, deployment, refunds/proration/customer portal/multi-plan billing, production Stripe account/Price setup, production S3 storage wiring, full deployment wiring, and QA product behavior scheduled for later steps
...
- Added a pinned backend PDF runtime Dockerfile using `node:22.23.1-bookworm-slim`, Debian `chromium=151.0.7922.173-1~deb12u1`, `PDF_EXPORT_CHROMIUM_EXECUTABLE_PATH=/usr/bin/chromium`, and a root `.dockerignore` that excludes `.env` and generated/dependency folders from Docker build context.
