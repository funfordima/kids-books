...
- Secure PDF export contracts under `apps/backend/src/pdf-exports`, including authenticated export/download routes, Step 8 entitlement checks, ready-book snapshot validation, deterministic content/layout versions, in-process idempotency plus database-backed duplicate-render avoidance, escaped semantic print HTML, deterministic image fallbacks, a hardened browser-renderer port that disables JavaScript and aborts external requests, private owner/book-scoped storage keys, short-lived signed download responses, sanitized failure mapping, and fake-port tests
...
- Added `PdfExportModule` with authenticated `POST /books/:bookId/pdf-exports` and `GET /books/:bookId/pdf-exports/:exportId/download` routes, Step 8 entitlement enforcement, ready-book snapshot validation, content hashing, escaped semantic print HTML, deterministic image fallbacks, in-process idempotency, database-backed duplicate-render avoidance, private object key construction, sanitized failure handling, and reauthorized short-lived downloads.
...
- Added fake-port tests for fail-closed entitlement/ownership/readiness, cache reuse, fallback rendering, private metadata, invalid page ordering/text, render failure cleanup, browser lockdown behavior, download reauthorization, and oversized image limits.
- Scope boundary: Step 9 does not implement AI regeneration, billing changes, public-template publication, reader redesign, social/public sharing, production deployment, real S3/MinIO adapter wiring, or a bundled Chromium runtime. Default renderer/storage adapters fail closed until production infrastructure is configured.
- Verification status: static diff and secret scans passed locally. Runtime gates are pending because `npm --workspace apps/backend test -- pdf-renderer.test.ts pdf-export.service.test.ts pdf-export.controller.test.ts` timed out through the repo-local Node toolchain, `node_modules/.bin/vitest` has no Windows command shim in this workspace, and Docker Desktop is not running; full Tester verification remains required before PR/review/merge.
